# Polis — Academic Integrity Policy

## Core Principles

1. **No fabricated citations.** The system must never generate citations that do not exist in the uploaded source base.
2. **No invented page numbers.** Every page reference must come from actual extracted text.
3. **No misattribution.** The system must not claim a source says something it does not say.
4. **Clear labelling.** Every AI output must distinguish between source-supported claims, model interpretation, user ideas, and general context.
5. **Sufficient evidence requirement.** The system must warn when the available source base is insufficient to support a claim.
6. **Student responsibility.** The student remains fully responsible for the final submitted work.
7. **Learning support, not replacement.** All features are designed to support understanding and improve academic skills.

## Label Categories

Every claim in an AI response must be labelled:

| Label | Meaning |
|-------|---------|
| **Source-supported** | Directly supported by uploaded source text, with citation |
| **Interpretation** | The model's reading of a source — reasonable but not explicitly stated |
| **User idea** | The student's own claim or argument |
| **General context** | Background knowledge, not from any specific uploaded source |
| **Unsupported** | Claim lacks sufficient evidence in the current source base |

## Prohibited Behaviours

The system will NOT:
- Generate complete essays for submission
- Fabricate citations, authors, or publications
- Invent page numbers or quote locations
- Present model-generated text as source text
- Help students circumvent university academic integrity policies
- Write content designed to pass AI detection tools

## Safe Behaviours

The system WILL:
- Help students understand dense readings
- Generate structured summaries with source references
- Identify key concepts and arguments in uploaded materials
- Compare theories using evidence from sources
- Help plan essay structures with evidence allocation
- Provide feedback on draft quality against rubrics
- Flag unsupported claims in student writing
- Suggest where additional sources are needed
- Check citation coverage in drafts

## Citation Rules

1. **Default citation style**: Harvard (author-date). Users may select APA, Chicago, or MLA in preferences.
2. All source-grounded claims must include: author, year, and page range where available
2. When a source is quoted, the original text must be shown alongside the citation
3. If the system cannot locate a specific page reference, it must say so explicitly
4. Paraphrased claims must still reference the source they draw from
5. "I couldn't find this in your sources" is always an acceptable answer

## Draft Feedback Boundaries

- Draft feedback identifies strengths, weaknesses, and improvement areas
- Feedback does NOT rewrite passages for the student
- Feedback does NOT generate new content to insert into drafts
- Feedback references the marking rubric where available
- Feedback suggests types of evidence needed, not specific fabricated quotes

## Student Responsibility

- The student remains fully responsible for all submitted work
- AI-generated summaries, suggestions, and feedback are aids — not substitutes for reading and understanding
- Students must verify all AI outputs against original source texts before use
- Students should check their university's policy on AI-assisted tools
- Students should discuss their use of AI tools with tutors if required
- Harvard citation style is the default; students should follow their department's required style

## University Policy

Students are encouraged to:
- Check their university's policy on AI-assisted tools
- Use Polis as a learning aid, not a writing substitute
- Cite all sources properly in final submissions
- Verify all AI-generated summaries against original texts
- Discuss their use of AI tools with tutors if required
